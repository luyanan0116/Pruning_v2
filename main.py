...
def main():
    ...
    parser.add_argument(
        ...
        choices=[
            ...
            "search", "lcb", "paper_mi", "paper_mi50", "paper_mi_gb", "paper_mi_gb_lcb",
        ],
    )
    ...
    parser.add_argument(
        "--paper_mi50_guidance_strength",
        type=float,
        default=0.001,
        help=(
            "MI-only low-PPL profile: near-one multiplier used only after the "
            "proposal-aligned MI score is computed; must be >0 to keep MI active"
        ),
    )
    ...
    if args.prune_method in PAPER_METHODS:
        ...
        if args.prune_method == "paper_mi50":
            if args.paper_mask_style != "wanda_weight":
                parser.error("paper_mi50 requires --paper_mask_style wanda_weight")
            if not args.paper_wanda_sequential:
                parser.error("paper_mi50 requires sequential Wanda calibration")
            if targets != {"mlp", "attention"}:
                parser.error(
                    "paper_mi50 requires --paper_prune_targets mlp,attention so the "
                    "reported transformer linear-weight sparsity is exactly 50%"
                )
            ratios = (float(args.sparsity_ratio), float(mlp_ratio), float(attn_ratio))
            if any(abs(value - 0.5) > 1e-12 for value in ratios):
                parser.error(
                    "paper_mi50 hard-requires sparsity_ratio, mlp_sparsity_ratio and "
                    "attention_sparsity_ratio to resolve to exactly 0.5"
                )
            if args.paper_mi50_guidance_strength <= 0:
                parser.error("--paper_mi50_guidance_strength must be > 0")
        if args.paper_mask_style == "wanda_weight":
