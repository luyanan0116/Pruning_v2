...
def prune_lcb(args, model, tokenizer, device=torch.device("cuda:0"), prune_n=0, prune_m=0):
    """Backward-compatible alias for the paper-aligned MI+GB+LCB pipeline."""
    from .prune_paper import prune_paper

    original_method = args.prune_method
    try:
        args.prune_method = "paper_mi_gb_lcb"
        return prune_paper(args, model, tokenizer, device, prune_n=prune_n, prune_m=prune_m)
    finally:
        args.prune_method = original_method
