...
from .paper_pruning.mi import build_frequency_spectrum
...
PAPER_METHODS = {"paper_mi", "paper_mi50", "paper_mi_gb", "paper_mi_gb_lcb", "lcb"}
...
def _canonical_method(method: str) -> str:
    return "paper_mi_gb_lcb" if method == "lcb" else ("paper_mi" if method == "paper_mi50" else method)
...
def _config_payload(args, config: PipelineConfig, cache, targets: tuple[str, ...]) -> dict:
    payload = {
        ...
        # Mask-application controls are intentionally excluded. They do not alter
        # gradient responses, MI, band merging, coverage selection or unit scores,
        # and excluding them lets a low-PPL guidance sweep reuse the same MI cache.
...
def _mi50_score_payload(args, config: PipelineConfig, cache, targets: tuple[str, ...]) -> dict:
    payload = {
...
def _score_mi50_or_load(
    ...
    """Compute only the proposal-aligned global MI stage for paper_mi50.

    Granular-ball and LCB code is deliberately not executed in this ablation.
    Frequency-band coverage and the hard 50% deployment budget remain active.
    """
    config = _pipeline_config(args)
    payload = _mi50_score_payload(args, config, cache, targets)
    ...
    if indices_path.exists() and unit_scores_path.exists() and config_path.exists() and not args.paper_overwrite_scores:
        ...
        if previous == payload:
            print(f"reusing MI-only scores from {report_dir}")
            return _load_selection_file(indices_path), _load_unit_scores(unit_scores_path)
        print("MI-only score configuration changed; recomputing scores")

    selections: Dict[str, Dict[str, Dict[int, np.ndarray]]] = {
        "paper_mi": {unit_type: {} for unit_type in targets}
    }
    ...
    fields = [
        "unit_type", "layer", "unit", "mi_total", "coverage_priority", "pruned"
    ] + [f"band_mi_{band}" for band in range(band_count)]
    score_path = report_dir / "mi_only_contribution_scores.csv"
    boundary_path = report_dir / "frequency_band_boundaries.csv"
    with score_path.open("w", newline="", encoding="utf-8") as score_handle, boundary_path.open(
        "w", newline="", encoding="utf-8"
    ) as boundary_handle:
        score_writer = csv.DictWriter(score_handle, fieldnames=fields)
        score_writer.writeheader()
        boundary_writer = csv.DictWriter(
            boundary_handle,
            fieldnames=[
                "unit_type", "layer", "band", "start_fine_bin", "end_fine_bin",
                "probe_count", "probe_indices", "fine_relevance", "achieved_coverage",
            ],
        )
        boundary_writer.writeheader()
        for layer_id in range(cache.num_layers):
            print(f"[paper MI-only scoring] layer {layer_id + 1}/{cache.num_layers}")
            for unit_type in targets:
                responses = np.asarray(cache.load_layer(layer_id, unit_type), dtype=np.float32)
                spectrum = build_frequency_spectrum(responses, cache.events, config.frequency)
                ratio, exact_count = _target_budget(args, unit_type)
                prune_count = resolve_prune_count(spectrum.total_mi.size, ratio, exact_count)
                keep_count = spectrum.total_mi.size - prune_count
                keep, priority, achieved = coverage_aware_keep_indices(
                    spectrum.total_mi, spectrum.band_mi, keep_count, config.budget
                )
                prune = np.setdiff1d(
                    np.arange(spectrum.total_mi.size, dtype=np.int64), keep, assume_unique=True
                )
                selections["paper_mi"][unit_type][layer_id] = prune
                unit_scores["paper_mi"][unit_type][layer_id] = priority.astype(np.float32)
                pruned_set = set(prune.tolist())
                for unit in range(spectrum.total_mi.size):
                    row = {
                        "unit_type": unit_type,
                        "layer": layer_id,
                        "unit": unit,
                        "mi_total": float(spectrum.total_mi[unit]),
                        "coverage_priority": float(priority[unit]),
                        "pruned": unit in pruned_set,
                    }
                    for band in range(band_count):
                        row[f"band_mi_{band}"] = float(spectrum.band_mi[unit, band])
                ...
                for band, (start, end) in enumerate(spectrum.band_ranges):
                    boundary_writer.writerow({
                        ...
                        "end_fine_bin": end,
                        "probe_count": int(spectrum.probe_indices.size),
                        "probe_indices": json.dumps(spectrum.probe_indices.tolist()),
                        "fine_relevance": json.dumps(spectrum.fine_relevance[start:end].tolist()),
                        "achieved_coverage": float(achieved[band]),
                    })
                print(
                    f"  {unit_type}: units={spectrum.total_mi.size}, prune={prune_count}, "
                    f"ratio={prune_count / spectrum.total_mi.size:.4f}, coverage="
                    + "/".join(f"{value:.2f}" for value in achieved),
                    flush=True,
                )
                del responses, spectrum
...
def _score_or_load(
    ...
    if args.prune_method == "paper_mi50":
        return _score_mi50_or_load(args, cache, report_dir / "mi50_scores", targets)
    config = _pipeline_config(args)
...
def _assert_exact_half_weight_masks(summaries) -> None:
    total_zeros = 0
    total_weights = 0
    for row in summaries:
        before = int(row["zeros_before"])
        after = int(row["zeros_after"])
        weights = int(row["rows"]) * int(row["columns"])
        if before != 0:
            raise RuntimeError(
                f"paper_mi50 expects an unpruned model, but {row['module']} already "
                f"contains {before} zero weights"
            )
        expected = weights // 2
        if weights % 2 or after != expected:
            raise RuntimeError(
                f"paper_mi50 exact-50% check failed for {row['module']}: "
                f"zeros={after}, expected={expected}, weights={weights}"
            )
        total_zeros += after
        total_weights += weights
    if total_weights == 0 or total_zeros * 2 != total_weights:
        raise RuntimeError(
            f"paper_mi50 global exact-50% check failed: {total_zeros}/{total_weights}"
        )
    print(
        f"paper_mi50 hard sparsity check passed: {total_zeros}/{total_weights} = 0.500000",
        flush=True,
    )
...
def prune_paper(args, model, tokenizer, device=torch.device("cuda:0"), prune_n=0, prune_m=0):
    ...
    mi50_profile = args.prune_method == "paper_mi50"
    ...
    use_sequential = args.paper_wanda_sequential
    if mi50_profile:
        # Low-PPL guard: preserve the proposal-aligned MI score, but make it a
        # small tie-breaker around the sequential Wanda operating point. Fixed
        # row budgets eliminate unit-wise sparsity redistribution and keep every
        # transformer linear matrix at exactly 50% zeros.
        mlp_ratio = 0.5
        attention_ratio = 0.5
        row_spread = 0.0
        guidance_strength = float(args.paper_mi50_guidance_strength)
        use_sequential = True
        print(
            "paper_mi50 profile: proposal MI + frequency coverage, sequential Wanda, "
            f"row_spread=0, guidance_strength={guidance_strength:.6g}, exact sparsity=0.5"
        )
    print(
    ...
    if mi50_profile:
        _assert_exact_half_weight_masks(summaries)
    summary_name = "weight_mask_summary_paper_mi50.csv" if mi50_profile else f"weight_mask_summary_{method}.csv"
